...
export function transitionChallengeState(state, progress, now = new Date()) {
  ...
  return normalizeChallengeState({
    ...
    ...nextProgress,
    completedAt:
      totalProgress >= TARGET ? current.completedAt || updatedAt : null,
    updatedAt,
  });
}
...
export function deriveChallenge(state, now = new Date()) {
  ...
  const completedBeforeDeadline =
    normalizedState.completedAt && endAt
