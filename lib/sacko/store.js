...
local timingIntent = ARGV[4]

if total >= tonumber(ARGV[2]) then
  -- The pre-EVAL candidate may be stale. Completion evidence must come from an
  -- explicit intent or the state currently in Redis.
  nextState["completedAt"] = cjson.null
  nextState["completionTiming"] = cjson.null

  local currentState = nil

  if currentRaw then
    currentState = cjson.decode(currentRaw)
  end

  if timingIntent == "before-deadline" or timingIntent == "after-deadline" then
    nextState["completionTiming"] = timingIntent
  elseif timingIntent == "preserve" and currentState then
    local currentTiming = currentState["completionTiming"]
    if currentTiming == "before-deadline" or currentTiming == "after-deadline" then
      nextState["completionTiming"] = currentTiming
    end
  end

  if not nextState["completionTiming"] or nextState["completionTiming"] == cjson.null then
    if currentState
      and currentState["completedAt"]
      and currentState["completedAt"] ~= cjson.null then
...
export async function updateChallengeProgress(
  ...
  const normalizedCompletionTiming =
    completionTiming === undefined
      ? undefined
      : normalizeCompletionTiming(completionTiming);
  const timingIntent =
    normalizedCompletionTiming === undefined
      ? "preserve"
      : normalizedCompletionTiming || "clear";
  const updatedAt = timestamp(now);
  const config = storageConfig();

  if (config.type === "redis") {
    const current = await readRedis(config);
    const candidate = transitionChallengeState(
      current,
      validatedProgress,
      updatedAt,
      normalizedCompletionTiming,
    );
